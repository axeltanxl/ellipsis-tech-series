...
const Food = () => {
	...
	const formatMeals = (meals) => {
		return meals.map((meal) => {
			...
			return {
				name: mealType,
				date: time.split('T')[0],
				items: [{ food: food, sodium: sodiumAmount, sugar: sugarAmount }],
			};
		});
	};
	...
	const filteredMeals = formattedMeals
		.filter(
			(meal) => meal.date === formatDate(displayedDate.toLocaleDateString('en-US').split('/'))
		)
		.reverse();
	...
	return (
        ...
        />
								...
								<React.Fragment key={index}>
									<tr className="bg-indigo-100">
										...
										<td className="p-3 font-bold">
											{sumTotalSodium(
												filteredMeals.find(
													(meal) => meal.name === category.toUpperCase()
												)?.items || []
											)}
										</td>
										<td className="p-3 font-bold">
			...
			<div className="flex flex-col my-10 bg-white rounded-lg shadow-lg p-6">
				...
				<div className="flex flex-row items-center mb-2">
					...
					<ul className="flex flex-col items-start pl-8 xs:text-sm md:text-base lg:text-lg">
						...
						<li className="py-2">
							Your sodium and sugar consumption was higher in April than other months
							but is going down
						</li>
						<li className="py-2">
							You can continue to manage your kidney healthy by eating healthy and
							keeping active
						</li>
					</ul>
				</div>
			...
			<div>
				<FoodEntry
					open={openFoodEntry}
					setOpenFoodEntry={setOpenFoodEntry}
				/>
			</div>
		</Layout>
	);
